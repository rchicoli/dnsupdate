...
#
# Usage: dnsupdate -s server -k key -t ttl add _test01._tcp.mc.example.com SRV 0 0 25566 mc.example.com.
# -h HELP!
# -s the server
# -k the key
# -t the ttl
...
def getKey(FileName):
    ...
    algo = keyfile[1].strip().rsplit(' ')[1].replace(';','').replace('-','_').upper()
    key = keyfile[2].strip().rsplit(' ')[1].replace('}','').replace(';','').replace('"', '')
    k = {hostname:key}
    try:
...
def doUpdate(Server, KeyFile, Origin, TimeToLive, doPTR, myInput):
    ...
    Update = dns.update.Update(Origin, keyring=KeyRing, keyalgorithm=getattr(dns.tsig, KeyAlgo))
